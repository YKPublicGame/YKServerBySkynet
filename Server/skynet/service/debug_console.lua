...
function COMMAND.help()
	return {
		...
		snaxHotfix = "snaxHotfix serviceName address source",
	}
end
...
function COMMAND.snaxHotfix(serviceName,node,source)
	local bind
	if node == nil or node == "" then -- 当前节点
		bind =  snax.queryservice(serviceName)
	else
		bind = cluster.snax(node, serviceName)
	end
    local s =
    [[
	    function accept.testFunc2(msg)
            print(msg == nil )
            print(" msg ---------------- >>>>>>>>>>>>>>>>>  -------------------------- >>>>>>>>>>>>>>>" ,msg)
        end
	]]
	local tt = snax.hotfix(bind,s)
	bind.post.testFunc2("00000000000000 >>>>>>>>>>>>>>>>>> ")
	return tt
end
