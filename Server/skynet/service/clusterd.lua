...
function command.add_node(source, nodename, nodeaddr)
	if node_address[nodename] ~= nodeaddr then
		if rawget(node_channel, nodename) then
			node_channel[nodename] = nil
		end
		node_address[nodename] = nodeaddr
	end

	skynet.ret(skynet.pack(nil))
end

function command.remove_node(source, nodename)
	if rawget(node_channel, nodename) then
		node_channel[nodename] = nil
	end
	if rawget(node_address, nodename) then
		node_address[nodename] = nil
	end
	skynet.ret(skynet.pack(nil))
end
